...
class mysql_checker extends sae_checker{
    ...
    public function ck_insert( $for_other_method=false ){
        $this->need_table( 'm' );
        ...
        if( $result['selfchk_value']==$value && $for_other_method )
            return $key;
...
class kvdb_checker extends sae_checker{
    ...
    public function ck_pkrget(){
        $this->init();
        $keys = array();
        $prefix = 'saeck_pkrget_'.rand(10,99).'_';
        for($i=0;$i<10;$i++)
            $keys[] = $prefix.rand(1,999).'eE';
        foreach( $keys as $k ){
            $this->kv->delete( $k );
            $this->kv->set($k,$this->value);
        }
...
class weibo_checker extends sae_checker{
    public function ck_connect(){
        ...
        if( is_array($rep) && isset($rep['error']) && isset($rep['error_code']) && isset($rep['request']) )
            return '1.0';
        else
            return '1.1';
    }    
